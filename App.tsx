  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900 overflow-x-hidden">
      ...
      {isModalOpen && (
        ...
        >
          <button className="absolute top-6 right-6 text-white bg-white/20 p-3 rounded-full hover:bg-white/30 transition-colors z-[110]">
      ...
      {showSolution && (
        ...
        >
          <div className="bg-white p-8 md:p-12 rounded-[2rem] md:rounded-[3rem] max-w-2xl shadow-2xl border-4 border-blue-500" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-lg md:text-2xl text-slate-700 leading-relaxed font-bold italic">
              {SOLUTION_TEXT}
            </p>
            <button 
              onClick={() => setShowSolution(false)}
        ...
        <div className="flex flex-col xl:flex-row gap-6 lg:gap-10 h-full">
          ...
          <div className="w-full xl:w-[45%] h-auto xl:sticky xl:top-36">
            <div className="bg-slate-950 text-white rounded-[2.5rem] md:rounded-[3rem] shadow-2xl overflow-hidden flex flex-col h-full border border-slate-900">
               <div className="relative group cursor-pointer flex-grow min-h-[300px] md:min-h-[400px]" onClick={() => setIsModalOpen(true)}>
                  <img src={paintingUrl} onError={handleImageError} alt="Bildquelle" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                  ...
                  <div className="flex flex-col gap-5 h-full">
                     ...
                     {activeStep === 4 && (
                       <div className="bg-slate-900 p-8 rounded-[2.2rem] shadow-2xl border-4 border-slate-800 mb-4 animate-in slide-in-from-right duration-500">
                          <h4 className="text-white font-black uppercase text-center text-xs tracking-widest mb-6">Wähle die richtige Ampelfarbe:</h4>
                          <div className="flex justify-center gap-6">
                             <button 
                                onClick={() => setAmpelChoice('red')}
                                className={`w-16 h-16 md:w-20 md:h-20 rounded-full bg-red-600 border-4 transition-all active:scale-90 ${ampelChoice === 'red' ? 'border-white shadow-[0_0_25px_rgba(220,38,38,0.8)] scale-110' : 'border-red-900/50 opacity-40 grayscale hover:grayscale-0 hover:opacity-100'}`}
                             />
                             <button 
                                onClick={() => setAmpelChoice('yellow')}
                                className={`w-16 h-16 md:w-20 md:h-20 rounded-full bg-yellow-400 border-4 transition-all active:scale-90 ${ampelChoice === 'yellow' ? 'border-white shadow-[0_0_25px_rgba(250,204,21,0.8)] scale-110' : 'border-yellow-900/50 opacity-40 grayscale hover:grayscale-0 hover:opacity-100'}`}
                             />
                             <button 
                                onClick={() => setAmpelChoice('green')}
                                className={`w-16 h-16 md:w-20 md:h-20 rounded-full bg-green-500 border-4 transition-all active:scale-90 ${ampelChoice === 'green' ? 'border-white shadow-[0_0_25px_rgba(34,197,94,0.8)] scale-110' : 'border-green-900/50 opacity-40 grayscale hover:grayscale-0 hover:opacity-100'}`}
                             />
                          </div>
                          
                          {ampelChoice && (
                            <div className={`mt-8 p-6 rounded-2xl border-2 font-bold text-center animate-in zoom-in-95 duration-300 ${ampelChoice === 'yellow' ? 'bg-yellow-50 border-yellow-400 text-yellow-900' : 'bg-slate-800 border-slate-700 text-slate-300'}`}>
                     ...
                     <button 
                       ...
                       className={`w-full flex items-center justify-between p-7 md:p-8 rounded-[2.2rem] font-black uppercase tracking-widest transition-all shadow-lg text-lg border-b-8 active:scale-[0.98] ${showHints ? 'bg-amber-400 text-amber-950 border-amber-600' : 'bg-slate-900 text-white border-slate-700 hover:bg-slate-800'}`}
                     >
                       <span className="flex items-center gap-4">
                         <span className="text-3xl">{showHints ? '📖' : '🔍'}</span>
                         <span>Detektiv-Lupe</span>
                       </span>
                       <svg xmlns="http://www.w3.org/2000/svg" className={`h-8 w-8 transition-transform duration-300 ${showHints ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                       ...
                       <div className="bg-amber-50 p-6 rounded-[2.2rem] border-4 border-amber-200 shadow-inner animate-in slide-in-from-top-4 duration-300">
                          <div className="space-y-3">
                            {currentStep.hints.map((hint, i) => (
                              <div key={i} className="bg-white p-5 rounded-2xl border-2 border-amber-100 text-amber-950 font-black text-base md:text-lg flex items-center gap-5 shadow-sm">
                     ...
                     <button 
                       ...
                       className={`w-full flex items-center justify-between p-7 md:p-8 rounded-[2.2rem] font-black uppercase tracking-widest transition-all shadow-lg text-lg border-b-8 active:scale-[0.98] ${showWritingHelp ? 'bg-indigo-100 text-indigo-950 border-indigo-300' : 'bg-indigo-600 text-white border-indigo-900 hover:bg-indigo-700'}`}
                     >
                       <span className="flex items-center gap-4">
                         <span className="text-3xl">{showWritingHelp ? '✍️' : '💡'}</span>
                         <span>Schreib-Hilfe</span>
                       </span>
                       <svg xmlns="http://www.w3.org/2000/svg" className={`h-8 w-8 transition-transform duration-300 ${showWritingHelp ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                         <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={4} d="M19 9l-7 7-7-7" />
                       </svg>
                     </button>
                       ...
                       <div className="bg-indigo-50 p-6 rounded-[2.2rem] border-4 border-indigo-100 shadow-inner animate-in slide-in-from-top-4 duration-300">
                          <div className="space-y-3">
                            {currentStep.sentenceStarters.map((s, i) => (
                              <div key={i} className="bg-white p-5 rounded-2xl border-2 border-indigo-100 text-indigo-950 font-black text-base md:text-lg italic shadow-sm leading-tight">
                                „{s}“
                              </div>
                            ))}
                          </div>
